...
"""Simulation of the model-free decision bias agent."""
...
with open(f'saved/opti_results/mfree_dbias.json', 'r') as json_file:
...
# Load agent function
agent_function = mf_policy

## SIMULATION ##
print(f"MF Decision Bias simulations with {world_model} world model")
...
if save:
    ...
    with open(f'saved/{world_model}/mfree_dbias_{world_model}.json', 'w') as json_file:
        json.dump(data, json_file, indent=4)
    np.savez_compressed(f'saved/{world_model}/dbias_mfree_value_epi.npz', *value_epi_saved)
    print("MF Social DB data saved")

    ## PLOT PERFORMANCE ##
    fig1, ax1 = plot_performance(sum_rewards, "Episodes", f"MF DB {params['omega']} omega", "Performance", n_episodes*training )
    fig1.savefig(f'saved/figures/{world_model}/mf_dbias_{world_model}_performance.png')
    fig2, ax2 = plot_performance(steps_to_reward, "Episodes", f"MF DB {params['omega']} omega", "Steps to reward", n_episodes*training )   
    fig2.savefig(f'saved/figures/{world_model}/mf_dbias_{world_model}_steps.png')
    print("Figures saved")